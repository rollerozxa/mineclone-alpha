...
local function register_wires()
	...
	while true do
		...
		local groups_on = {dig_immediate = 3, mesecon_conductor_craftable = 1,
			not_in_creative_inventory = 1, attached_node = 1, dig_by_water = 1,destroy_by_lava_flow=1}
		local groups_off = {dig_immediate = 3, mesecon_conductor_craftable = 1,
			attached_node = 1, dig_by_water = 1,destroy_by_lava_flow=1, craftitem = 1}
		if nodeid ~= "00000000" then
		...
		local wirehelp, img, desc_off, desc_on
		if nodeid == "00000000" then
